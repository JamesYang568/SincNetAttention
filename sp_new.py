...
from am import AdditiveMarginSoftmax
...
output_folder = options.output_folder + '/' + options.m + '_' + options.s
...
# loss function
if (options.loss == 'AMSoftmax'):
    cost = AdditiveMarginSoftmax(margin=float(options.m))
elif (options.loss == 'ArcFace'):
    cost = AngularPenaltySMLoss(128, 462, 'arcface', m=float(options.m), s=float(options.s))
else:
    cost = nn.NLLLoss()
